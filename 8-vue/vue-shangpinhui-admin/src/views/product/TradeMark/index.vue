<template>
  <div>
    <!--按钮-->
    <el-button style="margin: 10px 0;" type="primary" icon="el-icon-plus" @click="showDialog">添加</el-button>
    <!--
         表格组件
    ...
    <el-pagination
      ...
      @size-change="handleSizeChange"
    />
    <!-- 添加品牌|或者修改品牌按钮点击的时候,弹出的对话框 -->
    <!--
      el-dialog(对话框)：
      title,左上角标题
      visible,控制对话框显示与隐藏（布尔值）
      el-form:表单组件,经常与el-form-item一起使用
      el-form-item:之间是有间距的
      label:设置前面文字
      label-width:设置文字的宽度
...
export default {
  ...
  data() {
    return {
      ...
      tmForm: { // 收集品牌信息:对象身上的属性，不能瞎写，需要看文档的
        tmName: '', // 品牌的名字
        logoUrl: '' // 品牌的图片
      },
      // 上传图片使用时的
  ...
  methods: {
    ...
    async addOrUpdateTradeMark() {
      ...
      // 发请求（添加品牌|修改品牌）
      const result = await this.$API.tradeMark.reqAdOrUpdateTradeMark(this.tmForm)
      if (result.code === 200) {
        // 弹出信息:添加品牌成功、修改品牌成功
        this.$message({
          type: 'success',
        ...
        await this.getPageList(this.tmForm.id ? this.page : 1)
      } else {
